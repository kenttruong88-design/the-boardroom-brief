...
Each of clips 2-5 (NOT the hook — see above) also needs a SHORT VISUAL SCENE description — a real-world setting/pose for the speaker in that clip, used to generate a distinct start frame per clip so no two clips show the same static shot. Keep each to one photographable sentence: a location plausible for the clip's content, a vertical FULL-BODY framing as if the viewer's own camera is filming her directly (handheld feel, natural angle — NEVER describe her holding or looking at a phone/camera herself, that breaks the illusion she's talking to the viewer), and what the speaker is doing. Her face and shoulders must stay toward the camera with direct eye contact in every scene — NEVER turned away, glancing off-camera, or caught mid-turn — that's the one fixed rule. Everything else should feel alive, not posed: give her a SPECIFIC, EXPRESSIVE hand gesture suited to what she's about to say (one hand raised mid-point, an open-palm gesture, counting off on fingers, gesturing toward something in the scene) and a natural relaxed stance — weight settled on one leg, a slight easy angle to the body is fine, since it's her face and eye line that must stay locked on camera, not her whole body squared up like a passport photo. A still photo can't show speech, so capture her in a natural paused/mid-gesture beat, not talking. Vary all four scenes from each other through LOCATION, props, stance, and — especially — a distinct hand gesture per clip; the one thing that must NOT vary is her facing the camera with eye contact. Ground each country's scenes in something evocative of that country/topic where sensible (e.g. a gym for a fitness article), not just "outside".
