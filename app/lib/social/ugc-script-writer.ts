...
Clip 1 — Hook (max 26 words): The article's own headline, lightly adapted so it reads as something Suki would say out loud rather than a written headline — keep its wit and contrast fully intact, do not soften or generalize it — followed by ONE short follow-up line that names BOTH countries and previews the Do's/Don'ts format to come, e.g. "Here's what to do — and what not to — in Ireland and Japan." (using that video's actual two countries, not these examples). Keep the follow-up in Suki's wry, curious voice rather than reciting that exact sentence every time, but it must always (a) name both countries and (b) signal the Do's/Don'ts framing — not a vague tease like "And I've lived both." This clip does NOT get its own visual scene — it always opens on the same fixed office backdrop shared by every video, so write only the spoken script for it.
...
Each of clips 2-5 (NOT the hook — see above) also needs a SHORT VISUAL SCENE description — a real-world setting/pose for the speaker in that clip, used to generate a distinct start frame per clip so no two clips show the same static shot. Keep each to one photographable sentence: a location plausible for the clip's content, a vertical FULL-BODY framing as if the viewer's own camera is filming her directly (handheld feel, natural angle — NEVER describe her holding or looking at a phone/camera herself, that breaks the illusion she's talking to the viewer), and what the speaker is doing. Her face and shoulders must stay toward the camera with direct eye contact in every scene — NEVER turned away, glancing off-camera, or caught mid-turn — that's the one fixed rule. Her stance and hands must be NEUTRAL and RELAXED, NOT mid-gesture: hands down at her sides, loosely clasped, or resting naturally — NEVER raised, NEVER already extended into a gesture, NEVER caught mid-point (the video model animates gestures from this frame throughout the clip; a start pose that's already at a gesture's peak leaves it nothing to do but slowly relax out of that one pose for the whole clip, which reads as static and unnatural, not lively). Weight settled on one leg, a slight easy angle to the body is fine, since it's her face and eye line that must stay locked on camera, not her whole body squared up like a passport photo. Vary the four scenes from each other through LOCATION, props, and stance angle — NOT through hand gesture, which should stay neutral in every scene and let the animation add movement instead. Ground each country's scenes in something evocative of that country/topic where sensible (e.g. a gym for a fitness article), not just "outside".
