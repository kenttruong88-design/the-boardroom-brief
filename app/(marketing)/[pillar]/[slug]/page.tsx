...
import { Clock, ArrowLeft, Share2, Bookmark, Linkedin, Twitter } from "lucide-react";
import { MOCK_ARTICLES, PILLARS, ECONOMIES, getArticleBySlug, formatDate, formatDateShort } from "@/app/lib/mock-data";
...
export function generateStaticParams() {
  return MOCK_ARTICLES.map((a) => ({ pillar: a.pillar, slug: a.slug }));
}

export default async function ArticlePage({ params }: Props) {
  const { pillar: pillarSlug, slug } = await params;
  const article = getArticleBySlug(slug);
  if (!article || article.pillar !== pillarSlug) notFound();

  const pillar = PILLARS.find((p) => p.slug === pillarSlug);
  const related = MOCK_ARTICLES.filter(
    (a) => a.pillar === pillarSlug && a.slug !== slug
  ).slice(0, 3);
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <article className="lg:col-span-2">
            ...
            <header className="mb-8">
              {pillar && (
                <span className={`pillar-badge text-2xs mb-4 inline-block ${pillar.color}`}>
                  {pillar.name}
                </span>
              )}
              ...
              <div className="flex items-center justify-between flex-wrap gap-4">
                ...
                <div className="flex xl:hidden items-center gap-2">
                  <button className="p-2 border transition-colors hover:border-red-accent" style={{ border: "1px solid var(--border)", color: "var(--ink-m)", borderRadius: "2px" }}>
                    <Share2 className="w-4 h-4" />
                  </button>
                  <button className="p-2 border transition-colors hover:border-red-accent" style={{ border: "1px solid var(--border)", color: "var(--ink-m)", borderRadius: "2px" }}>
                    <Bookmark className="w-4 h-4" />
            ...
            <div className="font-sans text-base leading-relaxed space-y-5" style={{ color: "var(--ink)" }}>
              ...
              <h2 className="text-xl font-serif font-bold mt-8" style={{ color: "var(--navy)" }}>
                What the data shows
              </h2>

              <p>
                For executives navigating capital allocation decisions, the message is clear: cost of capital is normalising at higher levels than the post-GFC era. Balance sheet discipline and cash flow visibility are being rewarded by investors in ways not seen since the early 2000s — a period most CFOs remember with the fondness usually reserved for wisdom teeth removal.
              </p>

              <p>
                Companies that locked in long-duration debt at pandemic-era rates have a meaningful competitive advantage. That window is now closed. The remaining question is not whether rates come down, but how slowly, and whether your competitors are better positioned for the wait.
              </p>
              ...
              <div className="border-t pt-10 mt-10" style={{ borderColor: "var(--border)" }}>
                <div className="p-10 text-center" style={{ background: "var(--navy)" }}>
                  <p className="eyebrow-gold mb-3" style={{ color: "var(--gold)" }}>
                    Subscriber Only
                  </p>
                  <h3
                  ...
                  <div className="flex gap-3 justify-center">
                    ...
                    <Link
                      ...
                      style={{ border: "1px solid rgba(255,255,255,0.25)", color: "var(--cream)" }}
                    >
                      Sign in
                    </Link>
                  </div>
            ...
            {/* Author card */}
            <div className="mt-10 p-6 border" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
              <div className="flex items-center gap-4">
                ...
                />
                <div>
                  <p className="font-serif font-bold" style={{ color: "var(--navy)" }}>
                    {article.author}
                  </p>
            ...
            <div className="mt-12 p-6 border" style={{ borderColor: "var(--border)" }}>
              ...
              <p className="text-sm font-sans mt-2" style={{ color: "var(--ink-m)" }}>
                Comments are available to subscribers. Sign in to join the conversation.
              </p>
          ...
          <aside className="space-y-8">
            ...
            <div className="lg:sticky lg:top-4 space-y-8">
                  ...
                  <div className="space-y-4">
                    {related.map((a) => (
                      <article key={a.slug} className="group border-b pb-4" style={{ borderColor: "var(--border)" }}>
                        <Link href={`/${a.pillar}/${a.slug}`}>
                          <p
                            className="text-sm font-serif font-bold leading-snug group-hover:opacity-70 transition-opacity"
                            style={{ color: "var(--navy)" }}
                          >
                            {a.title}
                          </p>
                          <p className="text-2xs mt-1" style={{ color: "var(--ink-m)", fontFamily: "var(--font-jetbrains)" }}>
                            {formatDateShort(a.publishedAt)}
                          </p>
                        </Link>
                      </article>
              ...
              <div>
                ...
                <div className="space-y-2" style={{ border: "1px solid var(--border)" }}>
                  {[
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-2xs" style={{ color: item.up ? "#16a34a" : "#dc2626", fontFamily: "var(--font-jetbrains)" }}>
                          {item.change}
                        </div>
                      </div>
                    </div>
              ...
              <div className="p-6" style={{ background: "var(--navy)" }}>
                ...
                <h3 className="font-serif font-bold mb-2" style={{ color: "var(--cream)" }}>
                  Get this in your inbox
                </h3>
                <p className="text-xs font-sans mb-4" style={{ color: "rgba(245,240,232,0.55)" }}>
